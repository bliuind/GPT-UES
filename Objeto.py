...
class AjusteDeCurvas:
    ...
    def DatosHTML(self, y):
        Rango = len(y);
        SumY = 0;   SumYY = 0;
        for i in range(Rango):
            SumY = SumY + y[i];
            SumYY = SumYY + (y[i] * y[i])
        Media = SumY / Rango;  Desviacion = 0; 
        VarianzaNM = ((SumYY) - ((SumY * SumY) / Rango )) / (Rango - 1)
        Herramienta = Estadistica()
        DesviacionNM = Herramienta.DesviacionNewton(VarianzaNM)
        DatoMax = y[0]; DatoMin = y[0];
        IndiceMax = 0;  IndiceMin = 0;
        Contador = 1;   ContadorTemp = 1;
        DatoModa = y[0] 
        for i in range(Rango):
            Desviacion = Desviacion + ((y[i] - Media) * (y[i] - Media))
        ...
        if(ContadorTemp > Contador):
            DatoModa = y[Rango-1];
        Desviacion = Desviacion / (Rango - 1);
        return Media,Desviacion,VarianzaNM,DesviacionNM,DatoMax,IndiceMax,DatoMin,IndiceMin,DatoModa

    def RegresionLinealHTML(self, y, Desv):
        x = list(range(len(y)))
        Rango = len(x); Desv = Desv * Rango
        ...
        for i in range(Rango):
            ...
            SumXY = SumXY + (x[i] * y[i]);
            SumXX = SumXX + (x[i] * x[i]);
        A1 = ((Rango * SumXY)-(SumX * SumY))/((Rango * SumXX)-(SumX * SumX));
        A0 = (SumY / Rango) - (A1 * (SumX / Rango));
        Temp = 0;
        for i in range(Rango):
            E = y[i] - (A0 + (A1 * x[i]))
            Temp = Temp + (E * E)
        CD = self.CoeficienteDeterminacion(Desv,Temp)
        CC = self.CoeficienteCorrelacion(CD)
        return(CD , CC)

    def InterpolacionLinealNoGraphDos(self, Datos, Indice, Rango):
        i = 0;
        while(i < Rango):
            #Es el inicio del array un elemento vacio""
            if(Datos[i] == "" and i == 0):
                #Tomamos esta posicion
                Posicion1 = i;
                #Mientras asi sea seguira buscando
